...
나의 책장에 있던 『미래학자의 통찰의 기술』이라는 책을 다시 읽다다.

아래 책을 읽으며 내가 기억하고 체득하기 위해 사용할 수 있는 내용을 정리했다.

- 명분과 실리 분석.
- 사실과 의견 구분.
- 관찰 가능한 데이터와 인간의 분석을 돕기 위한 지료 구분.
- 물리와 생물학 본질 파악 후, 새로운 접근 구상. 예) NASA 깨지지 않는 전구 개발, SpaceX no rocket legs
- 현상으로만 결론 도출 자제. 깊은 내면 존재 가능. 예) 조직 내 갈등, 조직원 성과 변화
- 시스템과 내면 개별 조직 이해 및 연관성 파악 . Butterfly Effect. 예) 금리인상과 부동산/주식/외환 시장
- 변화는 동력이 필요하다. 예) 인간, 사회 욕구
- 퓨쳐스휠(Futures Wheel)을 통해 1차, 2차 영향력 시각적 도출.
- 3가지 미래 존재. 다가올 미래, 뜻밖의 미래, 원하는 미래
- 준비 없는 임기응변은 위험. Quarterbck, 연사 모두 과거 학습된 뇌 사용.